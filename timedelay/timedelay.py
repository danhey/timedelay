...
class TimeDelay:
    ...
    def __init__(self, times, mags, freqs=None, fmin=None, fmax=None, **kwargs):
        self.times = times
        self.mags = mags
        ...
        if freqs is None:
            freqs = estimate_frequencies(
                self.times, self.mags, fmin=self.fmin, fmax=self.fmax, **kwargs
            )
    ...
    def periodogram(self, N=100000):
        ...
        fct = np.sqrt(4.0 / len(self.times))
    ...
    def unique_colors(self, n, cmap="hls"):
        colors = np.array(sns.color_palette(cmap, n_colors=n))[::-1]
        return colors

    def wavelet(
        self, fmin, fmax, windows=500, nfreq=1000, gwidth=1, ax=None, cmap="jet"
    ):
    ...
    def first_look(self, segment_size=10, **kwargs):
        ...
        for delay, color in zip(time_delays, colors):
            ax.plot(time_midpoints, delay, "o-", alpha=1, color=color)
            ax.set_xlabel("Time [BJD]")
